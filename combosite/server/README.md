...
3. For Atlas, create a database user and add your current IP address to the Atlas IP access list.
4. Run `npm run dev` from the `combosite` directory.

The server creates these collections and indexes automatically:

- `users` with a unique email index
- `sessions` with a unique token index
